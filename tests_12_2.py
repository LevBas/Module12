...
class TournamentTest(unittest.TestCase):
    ...
    def test_turn1(self):
        turn_1 = runner_and_tournament.Tournament(90, self.runner_1, self.runner_3)
        result = turn_1.start()
        self.assertTrue(result[list(result.keys())[-1]] == 'Ник')
        self.all_results['Тест первого раунда'] = result

    def test_turn2(self):
        turn_2 = runner_and_tournament.Tournament(90, self.runner_2, self.runner_3)
        result = turn_2.start()
        self.assertTrue(result[list(result.keys())[-1]] == 'Ник')
        self.all_results['Тест второго раунда'] = result

    def test_turn3(self):
        turn_3 = runner_and_tournament.Tournament(90, self.runner_1, self.runner_2, self.runner_3)
        result = turn_3.start()
        self.assertTrue(result[list(result.keys())[-1]] == 'Ник')
